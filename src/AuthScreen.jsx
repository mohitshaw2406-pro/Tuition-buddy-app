...
export default function AuthScreen({ onLogin }) {
  ...
  const handleLogin = async () => {
    ...
    if (isDemo) { setErr("⚙️ Please configure Firebase credentials."); setLoading(false); return; }
    try {
      ...
      if (snap.exists()) {
        ...
        if (data.approved === false) {
          onLogin({ uid: cred.user.uid, ...data, isAdmin: false, pendingApproval: true });
          return;
        }
        // ────────────────────────────────────────────────────────────
  ...
  const demoStudentLogin = () => onLogin({
    ...
    totalQuestions: 24, totalQuizzes: 3, approved: true
  });

  // ── Signup success → pending approval screen ─────────────────────────────
  if (signupDone) return (
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Segoe UI',system-ui,sans-serif", padding: 20 }}>
      <div style={{ width: "100%", maxWidth: 420, textAlign: "center" }}>
        ...
        <h2 style={{ color: C.text, fontWeight: 800, fontSize: 22, margin: "0 0 8px" }}>Account Bana Gaya!</h2>
        <p style={{ color: C.muted, fontSize: 14, margin: "0 0 20px", lineHeight: 1.6 }}>
          Tera account approval ke liye bheja gaya hai.<br />
          <strong style={{ color: C.gold }}>Coach approve karenge tab login ho payega.</strong><br />
          Thoda wait karo! 🙏
        </p>
        <button onClick={() => { setSignupDone(false); setTab("login"); setEmail(""); setPw(""); setName(""); }} style={{ background: `linear-gradient(135deg,${C.accent},#4f46e5)`, border: "none", borderRadius: 10, color: "#fff", padding: "11px 28px", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
          🔑 Login Page pe Jao
        </button>
      </div>
    </div>
    ...
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Segoe UI',system-ui,sans-serif", padding: 20 }}>
      <div style={{ width: "100%", maxWidth: 420 }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <h1 style={{ margin: 0, fontSize: 28, fontWeight: 800, background: `linear-gradient(135deg,${C.accent},${C.purple})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Tuition Buddy</h1>
        ...
        {isDemo && (
          <Card style={{ marginBottom: 16, borderColor: "#f59e0b44" }}>
            ...
            <p style={{ color: C.muted, fontSize: 12, margin: "6px 0 10px" }}>
              Replace <code style={{ color: C.accent }}>FIREBASE_CONFIG</code> in <code style={{ color: C.accent }}>firebase.js</code> with your credentials.
            </p>
        ...
        <Card>
          ...
          {tab === "signup" && (
            <div style={{ marginBottom: 14 }}>
              ...
              <select value={cls} onChange={e => setCls(e.target.value)} style={{ width: "100%", background: C.dim, border: `1px solid ${C.border}`, borderRadius: 8, color: C.text, padding: "10px 12px", fontSize: 14, fontFamily: "inherit" }}>
          ...
          {tab === "signup" && (
            <div style={{ background: `${C.gold}11`, border: `1px solid ${C.gold}33`, borderRadius: 8, padding: "8px 12px", marginBottom: 12 }}>
              <p style={{ color: C.gold, fontSize: 12, margin: 0 }}>⚠️ Account banane ke baad coach se approval lena hoga.</p>
            </div>
          )}
          ...
          {err && (
            <div style={{ background: "#7c1f1f44", border: "1px solid #ef444444", borderRadius: 8, padding: "8px 12px", color: "#fca5a5", fontSize: 13, marginBottom: 12 }}>{err}</div>
          )}
